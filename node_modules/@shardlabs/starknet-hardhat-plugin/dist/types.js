...
function iterativelyCheckStatus(txHash, starknetWrapper, gatewayUrl, feederGatewayUrl, resolve, reject) {
    return __awaiter(this, arguments, void 0, function* () {
        const statusObject = yield checkStatus(txHash, starknetWrapper, gatewayUrl, feederGatewayUrl).catch((reason) => {
            console.warn(reason);
            return undefined;
        });
        if (!statusObject) {
            ...
            setTimeout(iterativelyCheckStatus, constants_1.CHECK_STATUS_RECOVER_TIMEOUT, ...arguments);
        }
            ...
            // eslint-disable-next-line prefer-rest-params
            setTimeout(iterativelyCheckStatus, constants_1.CHECK_STATUS_TIMEOUT, ...arguments);
        }
    });
}
...
class StarknetContract {
    ...
    invokeOrCall(choice, functionName, args, options = {}) {
        var _a, _b, _c;
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.address) {
    ...
    invoke(functionName, args, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            const executed = yield this.invokeOrCall("invoke", functionName, args, options);
            const txHash = extractTxHash(executed.stdout.toString());
            return new Promise((resolve, reject) => {
                iterativelyCheckStatus(txHash, this.starknetWrapper, this.gatewayUrl, this.feederGatewayUrl, () => resolve(txHash), (error) => {
    ...
    call(functionName, args, options = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            const optionsCopy = JSON.parse(JSON.stringify(options, (_key, value) => typeof value === "bigint" ? value.toString() : value)); // copy because of potential changes to the object
            if (optionsCopy.blockNumber === undefined) {
                // using || operator would not handle the zero case correctly
                optionsCopy.blockNumber = constants_1.PENDING_BLOCK_NUMBER;
            }
            const executed = yield this.invokeOrCall("call", functionName, args, optionsCopy);
